...
const DCSLineA = () => {
  ...
  useEffect(() => {
    const iframe = document.querySelector('.dcs-a-panel iframe');
    if (iframe && zoom !== 100) {
  ...
  const loadLatestFile = async () => {
    setLoading(true);
    try {
      const file = await getLatestExcelFile('dcs-a');
      if (file) {
        setExcelData(file);
      }
    } catch (error) {
  ...
  return (
    <div className="dcs-a-page">
      <SimpleHeader />
      
      <div className="dcs-a-container">
        {loading ? (
          ...
          <div className="dcs-a-single-view">
            <div className="dcs-a-panel">
              <div className="panel-header">
                <div className="panel-header-left">
                  <Activity size={20} />
                  <h2>DCS Haftalık Rapor A Hattı</h2>
                </div>
                {excelData && (
